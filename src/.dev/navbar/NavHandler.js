class NavHandler {
    ...
    */
    generateContent(){
        ...
        let centerPane            = document.getElementById('customdevicemgr');
    ...
    listenToHash(){
        ...
        setInterval(()=>{
            if(window.location.hash !== this.storedHash){
                this.storedHash = window.location.hash;
                if(this.storedHash !== '#' + 'customdevicemgr'){
                    geotab.addin.customdevicemgr.blur();
                } else {
                    geotab.addin.customdevicemgr.focus(global.api, global.state);
                }
            }
        }, 100);
    }
